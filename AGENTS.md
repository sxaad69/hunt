...
- `hunt/gmgn/` — GMGN read-only CLI (`gmgn-cli`, demo key `gmgn_solbscbaseethmonadtron` when `HUNT_GMGN_API_KEY` unset). Node 22 at `/home/hunt/.local/node` on Linode. 8-min smart-money wallet seed only. Judge path is SQLite. Flow-quality `token info` on ACCEPT (fail-open).
...
- **Gates**: dust floor ≥50 SOL · A: ceiling ≤3000 SOL **and** curve fill ≥50% (`real_sol/85`, else `virtual−30`) · B: graduated vault FDV **3k–25k SOL** (Starbucks band; `b_band_high` terminal) · circulating top10>75% · Tracker snipers.totalPercentage>20 fail-closed · socials + survival model · serial_rugger (SQL fixed) · GMGN flow-quality on ACCEPT (fail-open if CLI miss): bundler>50% / rat>25% / insider>25% / rug>40% / fresh>85% veto (`gmgn_*`); smart-degen/KOL annotated · **must have tracked GMGN smart-money buy** (`no_smart_wallet` defer 20 min, then reject). Boost still flips soft rejects; **cannot override top10 / snipers / b_band / gmgn_**. Tracked wallets may enter **dust ≥30 SOL** (TENT 41); **<30 stays locked** (ZEC 1). Seed: 8-min `track smartmoney` (`*pump` ≥$40); tracked = 2+ mints; ACCEPT top-traders never promoted · WS/exec split Plan C (recv never awaits sell)
- **tracker_json**: paper_decisions stores Tracker `risk` object when Tracker was called.
- **Exits**: SL −20% pre-tier · bank 50% @ +40% · 25% @ +60% · breakeven floor between tiers · moon bag (25%) laddered trail: 30% <3x → 20% @3x → 12% @10x → 8% @50x · **no max_hold** (removed 2026-09-12: BEAST +7% for 6h then 11x after time-stop)
...
- 📄 **Paper 2026-09-12 GMGN**: Plan-B unlatched (demo key). Linode Node 22 + `gmgn-cli` 1.6.2. Discovery = PumpPortal only. GMGN = 8-min smart-money wallet seed; judge path SQLite-only. ACCEPT top-traders never promoted. Flow `token info` on ACCEPT, fail-open. Live frozen.
- 📄 **Paper 2026-09-12 smart gate**: 187 SL / 24h, 177 no smart signal. Tracked buy is now **required** (`no_smart_wallet`). Tracked may boost dust ≥30 SOL (TENT); <30 locked (ZEC). Live frozen.
